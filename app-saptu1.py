...
# BARU: Impor 'compress_dynamic_range' bersama dengan 'normalize'
...
# ######################################################
# ### BARU: Fungsi wav_to_mp3 dengan Kompresi Dinamis ###
# ######################################################
def wav_to_mp3(wav_path, mp3_path):
    try:
        ...
        # BARU: Terapkan kompresi dinamis
        # Ini akan meratakan volume, membuat bagian pelan lebih terdengar
        # dan bagian keras lebih terkontrol.
        compressed_audio = compress_dynamic_range(
            audio,
            threshold=-20.0,  # Mulai kompresi saat audio di atas -20dB
            ratio=4.0,        # Rasio kompresi 4:1
            attack=5.0,       # Reaksi cepat (ms)
            release=50.0      # Waktu rilis cepat (ms)
        )
        ...
        logger.info(f"MP3 created (with compression): {mp3_path.name}") # Log diubah
        return True
    except Exception as e:
