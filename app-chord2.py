...
<body class="bg-gray-100 min-h-screen flex flex-col items-center justify-center p-4 font-sans antialiased">
    <div class="bg-white p-8 rounded-lg shadow-xl w-full max-w-2xl border border-gray-200">
        ...
        <div id="audioSection" class="border-t border-gray-200 pt-6 mt-6">
            ...
            <!-- DEBUG: Info untuk memastikan elemen metadata ada -->
            <div class="mb-2 text-xs text-gray-500">
                <p>Info :</p>
            </div>
            
            <!-- Metadata Audio (dengan styling yang lebih jelas) -->
            ...
            <!-- DEBUG: Info untuk memastikan elemen tersedia -->
            <div class="mt-4 p-2 bg-yellow-50 border border-yellow-200 rounded text-xs text-yellow-800">
                <p><strong>Debug Info:</strong> Audio Section Active</p>
                <p>ID audioPlayer: <span id="debugAudioPlayerId">N/A</span></p>
                <p>ID genreDisplay: <span id="debugGenreDisplayId">N/A</span></p>
            </div>
        </div>
    </div>

    <!-- Script utama Anda harus dimuat di akhir body -->
    <!-- Ini adalah script lama Anda yang memuat script.js eksternal -->
    <!-- <script src="script.js"></script> --> 
    
    <!-- Ini adalah script inline baru Anda -->
    <script>
        // Inisialisasi Wavesurfer di luar agar bisa diakses jika diperlukan
        let wavesurfer = null;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('debugAudioPlayerId').textContent = audioPlayer.id;
            document.getElementById('debugGenreDisplayId').textContent = genreDisplay.id;
            ...
            // Ganti generateBtn.addEventListener('click', ...) menjadi musicForm.addEventListener('submit', ...)
            musicForm.addEventListener('submit', async function(e) {
                e.preventDefault(); // Mencegah submit form default

                const formData = new FormData();
                formData.append('lyrics', lyricsInput.value.trim());
                formData.append('genre', document.getElementById('genreSelect').value); // Gunakan genreSelect
                formData.append('tempo', document.getElementById('tempoInput').value === '' ? 'auto' : document.getElementById('tempoInput').value); // Gunakan tempoInput

                // Reset UI
                statusMsg.textContent = '🚀 Memulai generasi instrumental...';
                generateBtn.disabled = true;
                loadingSpinner.classList.remove('hidden');
                audioSection.style.display = 'none'; // Sembunyikan hasil sebelumnya
                audioPlayer.style.display = 'none';
                downloadBtn.disabled = true;
                if (wavesurfer) {
                    wavesurfer.destroy(); // Hancurkan instance wavesurfer sebelumnya
                    wavesurfer = null;
                }
                ...
                try {
                    ...
                    const timeoutId = setTimeout(() => {
                        controller.abort();
                        statusMsg.innerHTML = `
                            <p class="text-red-600">❌ Permintaan melebihi batas waktu. Server mungkin masih memproses, harap tunggu atau coba lagi nanti.</p>
                    ...
                    }, 5 * 60 * 1000); // Set timeout menjadi 5 menit (5 * 60 * 1000 milidetik)
                    ...
                    if (response.ok) {
                        ...
                        if (data.success) {
                            ...
                            wavesurfer.load(audioPlayer.src); // Load dari audioPlayer.src
                            
                            wavesurfer.on('ready', () => {
                                console.log('Wavesurfer ready!');
                                wavesurfer.play(); // Auto-play setelah Wavesurfer siap
                            });
                            ...
                            // Autoplay audio element, in case wavesurfer doesn't
                            setTimeout(() => {
                                audioPlayer.play().catch(e => {
                                    console.warn('Autoplay blocked for audio element:', e);
                                });
                            }, 500);
                    ...
                    if (error.name === 'AbortError') {
                        // Pesan timeout sudah diatur oleh setTimeout
                    } else {
                        statusMsg.innerHTML = `
