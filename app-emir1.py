...
def find_best_instrument(choice_list, is_rock_metal=False):
    ...
    # Handle 'Power Chord' special case for rhythm in rock/metal
    if 'Power Chord' in choice_list and is_rock_metal:
        return 'Overdriven Guitar' # Power Chord is a technique, map to Overdriven Guitar

    for choice in choice_list:
        choice_lower = choice.lower().strip()
        
        # Specific overrides for rock/metal based on request
        if is_rock_metal:
            if 'gitar distorsi' in choice_lower: return 'Distortion Guitar'
            if 'organ' in choice_lower: return 'Rock Organ'
            if 'power chord' in choice_lower: return 'Overdriven Guitar' # Maps power chord to instrument
    ...
    # Fallback by category keywords if nothing matches
    ...
    return 'Acoustic Grand Piano' # Final fallback
...
def get_music_params_from_lyrics(genre, lyrics, user_tempo_input='auto'):
    ...
    # Duration beats will be calculated based on structure, not fixed
    return params
...
def generate_melody_section(params, section_beats, current_chord_progression):
    ...
    if "Power Chord" in params['instruments']['melody']: # Melody untuk rock/metal
        patterns = [[4], [2, 2], [8]] # Pola lebih sederhana untuk power chord
        velocities = [95, 110]
    ...
    while time_pos_beats < section_beats:
        ...
        # Pilih note dari scale yang cocok dengan chord saat ini jika ada
        ...
        # Cari nada yang ada di scale DAN di chord
        possible_pitches = [p for p in scale_notes if p % 12 in [n % 12 for n in current_chord]]
        if not possible_pitches: # Jika tidak ada yang cocok, gunakan dari scale saja
            possible_pitches = scale_notes

        octave_range = random.choice([0, 12]) # Batasi range octave untuk melody
        note_index = random.randint(0, len(possible_pitches) - 1)
        pitch = possible_pitches[note_index] + octave_range
        pitch = max(48, min(pitch, 84)) # Batasi pitch ke rentang yang wajar
        ...
        # Tambahkan efek ekspresif jika diaktifkan
        if add_expressive_effects and beat_duration >= 1.0: # Hanya untuk notes minimal 1 beat
             # Pitch bend - Vibrato atau Slide
            if random.random() < 0.3: # 30% kemungkinan vibrato
            ...
            elif random.random() < 0.1: # 10% kemungkinan slide-in
        ...
        # Reset pitch bend to 0 at the end of note if bend was applied
    ...
    # Final cleanup and sort for pitch bend events
...
def generate_rhythm_primary_section(params, section_beats, current_chord_progression):
    ...
    rhythm_data = []
    time_pos_beats = 0.0
    beats_per_chord = section_beats / len(current_chord_progression)
    
    is_power_chord = params['instruments']['rhythm_primary'] == 'Overdriven Guitar' # Jika Power Chord dipilih

    for i, chord_notes_midi in enumerate(current_chord_progression):
        chord_duration = beats_per_chord
        if time_pos_beats + chord_duration > section_beats:
            chord_duration = section_beats - time_pos_beats
            if chord_duration < 0.01: break
        
        # Velocity tergantung genre
        base_velocity = 80
        if params['genre'] in ['rock', 'metal']: base_velocity = 100
        elif params['genre'] == 'ballad': base_velocity = 70

        # Pattern untuk rhythm
        if is_power_chord: # Power chord (rock/metal)
            # Mainkan root dan fifth
            power_chord_notes = [chord_notes_midi[0], chord_notes_midi[0] + 7]
            if len(chord_notes_midi) > 1: # Tambah oktaf root jika ada
                 power_chord_notes.append(chord_notes_midi[0] + 12) 
            
            for _ in range(int(chord_duration / 0.5)): # Setiap half beat
                if time_pos_beats < section_beats:
                    for note in power_chord_notes:
                        velocity = random.randint(base_velocity, base_velocity + 20)
                        rhythm_data.append((note, time_pos_beats, 0.4, velocity))
                    time_pos_beats += 0.5 # Play every half beat
                else: break

        else: # Piano/Pad/String chords
            # Mainkan chord pada setiap beat atau half beat
            for _ in range(int(chord_duration / 1.0)): # Every beat
                 if time_pos_beats < section_beats:
                    for note in chord_notes_midi:
                        velocity = random.randint(base_velocity - 10, base_velocity + 10)
                        rhythm_data.append((note, time_pos_beats, 0.9, velocity)) # Longer duration for sustained feel
                    time_pos_beats += 1.0
                 else: break
...
def generate_rhythm_secondary_section(params, section_beats, current_chord_progression):
    ...
    for i, chord_notes_midi in enumerate(current_chord_progression):
        ...
        # Sustain chords for the full duration of the section
        for note in chord_notes_midi:
            velocity = random.randint(base_velocity - 5, base_velocity + 5)
            rhythm_data.append((note, time_pos_beats, chord_duration, velocity))
        
        time_pos_beats += chord_duration # Move to next chord start
...
def generate_bass_line_section(params, section_beats, current_chord_progression):
    ...
    for i, chord_notes_midi in enumerate(current_chord_progression):
        ...
        if bass_style == 'walking': # Jazz/Blues
            for beat in range(int(chord_duration)):
                if time_pos_beats + beat < section_beats:
                    note_to_play = root_note
                    # Tambahkan variasi walking bass
                    if beat % 4 == 1: note_to_play += random.choice([2, 3, 4, 5, 7]) # Second beat
                    elif beat % 4 == 2: note_to_play += random.choice([-1, 0, 1, 2, 3]) # Third beat
                    elif beat % 4 == 3: note_to_play -= random.choice([0, 1, 2]) # Fourth beat

                    velocity = random.randint(base_velocity - 10, base_velocity + 10)
                    bass_line_events.append((note_to_play, time_pos_beats + beat, 0.9, velocity)) # Long duration
        
        elif bass_style == 'driving': # Rock
            for beat in range(int(chord_duration / 1.0)): # Every beat
                if time_pos_beats + beat < section_beats:
                    velocity = random.randint(base_velocity, base_velocity + 15)
                    bass_line_events.append((root_note, time_pos_beats + beat, 0.7, velocity)) # Punchy
                    if random.random() < 0.3: # Tambah off-beat for drive
                         if time_pos_beats + beat + 0.5 < section_beats:
                            bass_line_events.append((root_note + random.choice([0, 5]), time_pos_beats + beat + 0.5, 0.4, velocity - 10))
        
        elif bass_style == 'heavy': # Metal
            for beat in range(int(chord_duration / 0.5)): # Every half beat
                if time_pos_beats + (beat * 0.5) < section_beats:
                    velocity = random.randint(base_velocity + 5, base_velocity + 20)
                    bass_line_events.append((root_note, time_pos_beats + (beat * 0.5), 0.4, velocity)) # Short, staccato
        
        elif bass_style == 'sustained': # Ballad
            velocity = random.randint(base_velocity - 20, base_velocity - 5)
            bass_line_events.append((root_note, time_pos_beats, chord_duration * 0.9, velocity)) # Long sustain
        
        elif bass_style == 'syncopated' or bass_style == 'tumbao' or bass_style == 'offbeat_syncopated': # Hiphop/Latin/Dangdut
            # Contoh tumbao/syncopated: mainkan root, lalu off-beat 5th/octave
            for beat in range(int(chord_duration / 1.0)):
                if time_pos_beats + beat < section_beats:
                    # Root on beat 1
                    bass_line_events.append((root_note, time_pos_beats + beat, 0.4, base_velocity))
                    
                    # Off-beat variations
                    if random.random() < 0.7:
                        if time_pos_beats + beat + 0.5 < section_beats:
                            bass_line_events.append((root_note + random.choice([0, 7, 12]), time_pos_beats + beat + 0.5, 0.4, base_velocity - 10))
                    
                    if random.random() < 0.5 and time_pos_beats + beat + 0.25 < section_beats:
                        bass_line_events.append((root_note + 12, time_pos_beats + beat + 0.25, 0.2, base_velocity - 20)) # Ghost note

        else: # Default: simple root notes
            for beat in range(int(chord_duration / 1.0)):
                if time_pos_beats + beat < section_beats:
                    velocity = random.randint(base_velocity - 10, base_velocity + 5)
                    bass_line_events.append((root_note, time_pos_beats + beat, 0.8, velocity))
...
def generate_drum_pattern_section(params, section_type, section_beats):
    ...
    drum_events = []
    base_velocity_kick = 120
    base_velocity_snare = 110
    base_velocity_hat = 80
    base_velocity_perc = 90
    
    # Adjust velocities based on section type
    if section_type in ['intro', 'outro', 'interlude']:
        base_velocity_kick -= 20
        base_velocity_snare -= 20
        base_velocity_hat -= 20
    elif section_type == 'chorus':
        base_velocity_kick += 5
        base_velocity_snare += 5
        base_velocity_hat += 5
    elif section_type == 'pre_chorus':
        base_velocity_kick -= 10
        base_velocity_snare -= 10
        base_velocity_hat -= 10
        
    for current_beat in range(0, int(section_beats)):
        
        # Kick drum (36)
        if current_beat % 4 == 0: # Beat 1
            drum_events.append((36, current_beat, 0.5, random.randint(base_velocity_kick, 127)))
        if current_beat % 4 == 2: # Beat 3
            drum_events.append((36, current_beat, 0.5, random.randint(base_velocity_kick - 10, base_velocity_kick)))
        
        # Snare drum (38)
        if current_beat % 4 == 1 or current_beat % 4 == 3: # Beats 2 & 4
            drum_events.append((38, current_beat, 0.4, random.randint(base_velocity_snare, 120)))

        # Hi-hat (42) / Ride (51)
        if section_type == 'chorus' or params['genre'] in ['jazz', 'metal']: # Use ride cymbal in chorus/jazz/metal
            for eighth in range(2):
                drum_events.append((51, current_beat + (eighth * 0.5), 0.3, random.randint(base_velocity_hat + 10, 110)))
            if section_type == 'chorus' and current_beat % 2 == 0: # Crash setiap 2 beat di chorus
                drum_events.append((49, current_beat, 1.0, 127)) # Loud crash
        else: # Hi-hat di bagian lain
            for eighth in range(4): # 16th notes for hi-hat
                drum_events.append((42, current_beat + (eighth * 0.25), 0.1, random.randint(base_velocity_hat - 10, base_velocity_hat + 10)))

        # Fills (optional)
        if random.random() < 0.1 and current_beat % 4 == 3 and current_beat < section_beats - 4: # Small fill near end of phrase
            drum_events.append((41, current_beat + 3.5, 0.25, random.randint(90, 110))) # Low Tom
            drum_events.append((43, current_beat + 3.75, 0.25, random.randint(90, 110))) # High Tom

    # Tambah crash di intro/outro
    if section_type == 'intro':
        drum_events.append((49, 0, 1.0, 127))
    if section_type == 'outro':
        drum_events.append((49, section_beats - 1, 1.0, 127))

    # Sort events by time
    drum_events.sort(key=lambda x: x[1])
    return drum_events

def generate_melody(params, section_beats, current_chord_progression):
    """Legacy generate_melody, now wrapper for generate_melody_section"""
    return generate_melody_section(params, section_beats, current_chord_progression)
...
def build_song_structure(params):
    ...
    song_structure = [] # List of (section_type, beats, chord_progression_for_section)
    ...
    base_beats = GENRE_PARAMS[params['genre']]['base_duration_beats_per_section']

    # --- INTRO ---
    song_structure.append(('intro', base_beats['intro'], chord_progression_main[:min(2, len(chord_progression_main))])) # Pendek
    ...
    while current_beats < target_min_duration_beats:
        # Verse 1
        song_structure.append(('verse', base_beats['verse'], chord_progression_main))
        current_beats += base_beats['verse']
        if current_beats >= target_min_duration_beats: break

        # Pre-Chorus
        song_structure.append(('pre_chorus', base_beats['pre_chorus'], chord_progression_main))
        current_beats += base_beats['pre_chorus']
        if current_beats >= target_min_duration_beats: break
        
        # Chorus
        song_structure.append(('chorus', base_beats['chorus'], chord_progression_main))
        current_beats += base_beats['chorus']
        if current_beats >= target_min_duration_beats: break

        loop_count += 1
        if loop_count == 1: # Setelah loop pertama, bisa ada interlude atau bridge
            if random.random() < 0.4: # 40% kemungkinan interlude
                song_structure.append(('interlude', base_beats['interlude'], random.choice(GENRE_PARAMS[params['genre']]['chord_progressions'])))
                current_beats += base_beats['interlude']
                if current_beats >= target_min_duration_beats: break
            elif random.random() < 0.3: # 30% kemungkinan bridge
                 song_structure.append(('bridge', base_beats['bridge'], chord_progression_bridge))
                 current_beats += base_beats['bridge']
                 if current_beats >= target_min_duration_beats: break

    # --- OUTRO ---
    # Pastikan ada outro, mungkin fade out
    if current_beats < target_min_duration_beats + base_beats['outro']: # Tambah outro jika belum cukup panjang
        song_structure.append(('outro', base_beats['outro'], chord_progression_main[0:1]))
        current_beats += base_beats['outro']
    
    params['duration_beats'] = current_beats # Update total duration beats
    logger.info(f"Song structure built: {len(song_structure)} sections, total beats: {current_beats}")
...
def create_midi_file(params, output_path):
    ...
    for section_type, section_beats, chord_progression_for_section in song_structure:
        logger.info(f"Generating section: {section_type} for {section_beats} beats at absolute beat {current_absolute_beat}")
        
        # Melody
        melody_events, pb_events = generate_melody_section(params, section_beats, chord_progression_for_section)
    ...
    last_tick_melody = process_events_for_track(melody_track, all_melody_messages + all_melody_pitch_bend_events)
