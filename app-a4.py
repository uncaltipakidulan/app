...
import hashlib
import shutil
import math  # Tambahan untuk bass line (ceil)
...
# Import pyfluidsynth dengan error handling (tetap, tapi tidak digunakan utama)
try:
...
# Inisialisasi Flask app (tetap sama)
app = Flask(__name__)
CORS(app, resources={r"/*": {"origins": "*"}})

# Path konfigurasi (tetap sama)
...
# Fungsi check_module dan check_python_dependencies (tetap sama)
...
# Dictionary instrumen GM SoundFont (Revisi: Tambah alias untuk match lebih baik, hindari warning)
INSTRUMENTS = {
    # Piano (tetap sama)
    ...
    # Chromatic Percussion (tetap sama)
    ...
    # Indonesian Instruments (tetap sama)
    'Gamelan': 114, 'Kendang': 115, 'Suling': 75, 'Rebab': 110,
    'Talempong': 14, 'Gambus': 25, 'Mandolin': 27, 'Harmonica': 77,
}

# Dictionary chord dan SCALES (tetap sama, skip untuk ringkas)
CHORDS = {
    # ... (sama persis dengan kode asli, tidak berubah)
...
# GENRE_PARAMS (tetap sama, skip untuk ringkas)
GENRE_PARAMS = {
    'pop': {
    ...
    # ... (sisa genre sama persis dengan kode asli)
    'rock': {
...
# Fungsi detect_genre_from_lyrics (tetap sama)
...
# Fungsi get_music_params_from_lyrics (Revisi: Perbaiki pemilihan instrumen dengan fuzzy match sederhana)
def get_music_params_from_lyrics(genre, lyrics, user_tempo_input='auto'):
    ...
    # Revisi: Pilih instrumen dengan fuzzy match (case-insensitive dan partial match)
    def find_best_instrument(choices):
        for choice in choices:
            # Case-insensitive exact match
            for instr, num in INSTRUMENTS.items():
                if choice.lower() ==():
                    return instr
            # Partial match jika exact gagal
            for instr, num in INSTRUMENTS.items():
    ...
    for category, instrument_choices in params['instruments'].items():
        selected = None
        if isinstance(instrument_choices, list):
            for choice in instrument_choices:
                selected = find_best_instrument([choice])
                if selected:
                    break
        else:
    ...
    # Generate chord progression (tetap sama)
...
# Fungsi get_scale_notes, generate_melody, generate_harmony (tetap sama, skip)
...
# Fungsi generate_bass_line (Revisi: Gunakan math.ceil untuk num_beats)
def generate_bass_line(params):
    ...
    for i, chord_notes in enumerate(chords):
        ...
        # Revisi: Gunakan math.ceil untuk num_beats
        num_beats = math.ceil(chord_duration)
        for beat_in_chord_segment in range(num_beats):
...
# Fungsi create_midi_file (tetap sama)
def create_midi_file(params, output_path):
    tempo = params['tempo']
    duration_beats = params['duration_beats']

    midi = MIDIFile(4, 120)  # 4 tracks untuk channel isolation
    ...
    if params['instruments']['melody']:
        ...
        for pitch, time_pos, duration, velocity in melody_notes:
            ...
            safe_duration = min(duration, duration_beats - time_pos - 0.1)
            if safe_duration > 0:
    ...
    if params['instruments']['harmony']:
        ...
        for pitch, start_beat, duration, velocity in harmony_data:
            ...
            safe_duration = min(duration, duration_beats - start_beat - 0.1)
            if safe_duration > 0:
    ...
    # Track 3: Drums (Channel 9, tetap sama)
...
# Fungsi midi_to_audio_pyfluidsynth (tetap sama, tidak digunakan utama)
...
# Fungsi midi_to_audio_subprocess (REVISI UTAMA: Fix endian untuk ARM64)
def midi_to_audio_subprocess(midi_path, output_wav_path, soundfont_path):
    try:
        # Revisi: Tambah -o untuk set endian='little' (untuk ARM64), format='s16', hilangkan -R 0 dan -E 0
        # Tambah -a null untuk skip audio driver (hanya render file, hindari ALSA/PulseAudio issue di root)
        cmd = [
            'fluidsynth',
            '-F', str(output_wav_path),  # Output WAV
            '-o', 'audio.file.endian=little',  # Fix utama: Set little-endian untuk ARM64
            '-o', 'audio.file.format=s16',     # Format stabil (signed 16-bit)
            '-o', 'synth.sample-rate=44100',   # Sample rate eksplisit
            '-a', 'null',                      # Skip audio driver (hanya file render)
        ...
        logger.debug(f"Menjalankan FluidSynth subprocess (ARM64 fixed): {' '.join(cmd)}")
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=300)

        if result.returncode == 0 and output_wav_path.exists() and output_wav_path.stat().st_size > 0:
            file_size = output_wav_path.stat().st_size / 1024
            logger.info(f"WAV generated sukses: {output_wav_path.name} ({file_size:.1f} KB)")
            return True
        else:
            logger.error(f"FluidSynth subprocess error (code: {result.returncode}): {result.stderr}")
            logger.error(f"STDOUT: {result.stdout}")
            if not soundfont_path.exists():
                logger.error(f"SoundFont tidak ditemukan: {soundfont_path}")
            if not midi_path.exists():
                logger.error(f"MIDI file tidak ditemukan: {midi_path}")
            # Tes manual: Sarankan command untuk debug
            logger.error(f"Debug: Coba jalankan manual: {' '.join(cmd)}")
            return False
        ...
        logger.error("Executable 'fluidsynth' tidak ditemukan. Install dengan: sudo apt install fluidsynth libsndfile1")
        return False
    except Exception as e:
        logger.error(f"Unexpected error in FluidSynth subprocess: {e}")
        return False

# Fungsi midi_to_audio (tetap panggil subprocess)
def midi_to_audio(midi_path, output_wav_path):
    return midi_to_audio_subprocess(midi_path, output_wav_path, SOUNDFONT_PATH)

# Fungsi wav_to_mp3 (tetap sama)
...
# Fungsi cleanup_old_files, generate_unique_id, routes (tetap sama, skip untuk ringkas)
...
# Routes: index(), generate_instrumental_endpoint(), serve_audio() (tetap sama persis dengan kode asli)
@app.route('/')
def index():
    # ... (HTML template sama persis, skip)
    html_template = """
    <!DOCTYPE html>
    <html>
    <head>
...
def generate_instrumental_endpoint():
    ...
    try:
        ...
        if not midi_to_audio(midi_path, wav_path):
            if midi_path.exists():
                midi_path.unlink()
            return jsonify({'error': 'Gagal konversi MIDI ke audio. Pastikan FluidSynth terinstal dan SoundFont benar. Cek log untuk detail.'}), 500
...
# Fungsi get_local_ip dan main_app_runner (Revisi: Tambah cek libsndfile untuk FluidSynth)
def get_local_ip():
    try:
        import socket
...
def main_app_runner():
    logger.info("Memulai Flask Generate Instrumental! (Revisi: Channel isolation + ARM64 FluidSynth fix)")
    ...
    try:
        ...
        libsndfile_path = shutil.which('libsndfile.so') or shutil.which('sndfile-config')  # Cek libsndfile untuk endian support
        ...
        logger.info(f"Support libsndfile (untuk WAV): {'Ya' if libsndfile_path else 'Tidak (install libsndfile1)'}")
        ...
        if not fluidsynth_path:
            ...
            sys.exit(1)
        if not ffmpeg_path:
            logger.critical("Executable 'ffmpeg' tidak ditemukan. Install")
            sys.exit(1)
        if not libsndfile_path:
            logger.warning("libsndfile tidak terdeteksi. Install dengan: sudo apt install libsndfile1 untuk support WAV lengkap.")
