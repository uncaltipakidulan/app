...
import shutil
import math
import operator  # Ditambahkan untuk menghindari konflik operator %
...
GENRE_PARAMS = {
    'pop': {
        ...
        'instruments': {
            'melody': ['Clean Electric Guitar', 'Flute', 'Overdriven Guitar', 'Soprano Sax'],
            'rhythm_primary': ['Electric Piano 1', 'Clean Electric Guitar'],
            'rhythm_secondary': ['String Ensemble 1', 'Warm Pad'],
    ...
    'rock': {
        ...
        'instruments': {
            'melody': ['Distortion Guitar', 'Rock Organ', 'Overdriven Guitar'],
            'rhythm_primary': ['Power Chord'],
    ...
    'metal': {
        ...
        'instruments': {
            'melody': ['Distortion Guitar', 'Orchestral Strings', 'Overdriven Guitar'],
            'rhythm_primary': ['Power Chord'],
    ...
    'ballad': {
        ...
        'instruments': {
            'melody': ['Nylon String Guitar', 'Violin', 'Overdriven Guitar', 'Flute'],
            'rhythm_primary': ['Electric Piano 1', 'Clean Electric Guitar'],
    ...
    'blues': {
        ...
        'instruments': {
            'melody': ['Tenor Sax', 'Jazz Electric Guitar', 'Overdriven Guitar', 'Harmonica'],
            'rhythm_primary': ['Electric Piano 1', 'Clean Electric Guitar', 'Drawbar Organ'],
    ...
    'jazz': {
        ...
        'instruments': {
            'melody': ['Tenor Sax', 'Vibraphone', 'Jazz Electric Guitar', 'Flute'],
    ...
    'hiphop': {
        ...
        'instruments': {
            ...
            'rhythm_primary': ['Poly Synth Pad', 'Clean Electric Guitar', 'Electric Piano 2'],
    ...
    'latin': {
        ...
        'instruments': {
            ...
            'rhythm_primary': ['Acoustic Grand Piano', 'Clean Electric Guitar'],
...
def find_best_instrument(choice_list, is_rock_metal=False):
    ...
    for choice in choice_list:
        ...
        if is_rock_metal:
            if 'gitar distorsi' in choice_lower or 'distortion guitar' in choice_lower: 
                return 'Distortion Guitar'
            if 'organ' in choice_lower: 
                return 'Rock Organ'
            if 'power chord' in choice_lower: 
                return 'Overdriven Guitar'
    ...
    logger.warning(f"No good instrument match for '{choice_list}', falling back to Acoustic Grand Piano.")
...
def generate_melody_section(params, section_beats, current_chord_progression, is_solo=False, add_expressive_effects=True):
    """Generates melody for a single section with expressive effects - FIXED"""
    scale_notes = get_scale_notes(params['key'], params['scale'])
    
    melody_events = []
    pitch_bend_events = []

    # Validasi input untuk debugging
    if not isinstance(current_chord_progression, list):
        logger.error(f"Invalid chord progression type: {type(current_chord_progression)}")
        current_chord_progression = [CHORDS['C']]  # Fallback
    
    # Pastikan semua chord adalah list of integers
    for i, chord in enumerate(current_chord_progression):
        if not isinstance(chord, list):
            logger.warning(f"Chord {i} is not a list: {chord}, using C major")
            current_chord_progression[i] = CHORDS['C']
        else:
            # Validasi bahwa semua note dalam chord adalah integer
            for j, note in enumerate(chord):
                if not isinstance(note, int):
                    logger.warning(f"Note {j} in chord {i} is not integer: {note}, using 60")
                    chord[j] = 60
    ...
    while time_pos_beats < section_beats:
        ...
        # FIXED: Hitung chord index dengan aman
        if len(current_chord_progression) > 0:
            ...
            # Fallback jika tidak ada chord progression
            current_chord = CHORDS['C']
            logger.warning("No chord progression available, using C major")
        
        # FIXED: Gunakan operator.mod untuk menghindari konflik dengan % string formatting
        try:
            # Extract chord root notes (modulo 12 untuk pitch class)
            chord_root_classes = [operator.mod(note, 12) for note in current_chord]
            # Filter scale notes yang cocok dengan chord root classes
            possible_pitches = [p for p in scale_notes if operator.mod(p, 12) in chord_root_classes]
        except Exception as e:
            logger.error(f"Error calculating possible pitches: {e}")
            # Fallback ke semua scale notes
        ...
        melody_events.append((pitch, time_pos_beats, beat_duration, velocity))
    ...
    # FIXED: Cleanup pitch bend events dengan error handling
    ...
    try:
        ...
        logger.error(f"Error cleaning pitch bend events: {e}")
        pitch_bend_events_cleaned = []
    
    logger.debug(f"Generated {len(melody_events)} melody events and {len(pitch_bend_events_cleaned)} pitch bend events")
...
def generate_rhythm_primary_section(params, section_beats, current_chord_progression):
    """Generates rhythm (piano/power chord) for a single section - FIXED dengan validation"""
    rhythm_data = []
    time_pos_beats = 0.0
    
    # Validasi chord progression
    if not isinstance(current_chord_progression, list) or len(current_chord_progression) == 0:
        logger.warning("Invalid chord progression for rhythm, using C major")
    ...
    for i in range(len(current_chord_progression)):
        ...
        # Validasi chord notes
        if not isinstance(chord_notes_midi, list) or len(chord_notes_midi) == 0:
            logger.warning(f"Invalid chord notes at index {i}, using C major")
            chord_notes_midi = CHORDS['C']
        ...
        if is_power_chord_rhythm: # Power chord (rock/metal)
            # Pastikan ada root note
            root_note = chord_notes_midi[0] if chord_notes_midi else 60
            power_chord_notes = [root_note, root_note + 7]
            if len(chord_notes_midi) > 1:
                 power_chord_notes.append(root_note + 12) 
            ...
            for beat_sub_div in range(int(chord_actual_duration / 0.5)): # Setiap half beat
                ...
                if current_sub_beat < section_beats:
                    for note in power_chord_notes:
                        ...
                        rhythm_data.append((safe_note, current_sub_beat, 0.4, velocity))
                else: 
                    break
            ...
            for beat_sub_div in range(int(chord_actual_duration / 1.0)): # Every beat
                 ...
                 if current_sub_beat < section_beats:
                    for note in chord_notes_midi:
                        ...
                        rhythm_data.append((safe_note, current_sub_beat, 0.9, velocity)) # Longer duration for sustained feel
                 else: 
                    break
...
def generate_rhythm_secondary_section(params, section_beats, current_chord_progression):
    """Generates secondary rhythm (pad/strings/organ) for a single section - FIXED"""
    rhythm_data = []
    time_pos_beats = 0.0
    
    # Validasi chord progression
    if not isinstance(current_chord_progression, list) or len(current_chord_progression) == 0:
        logger.warning("Invalid chord progression for secondary rhythm, using C major")
        current_chord_progression = [CHORDS['C']]
    ...
    for i in range(len(current_chord_progression)):
        ...
        # Validasi chord notes
        if not isinstance(chord_notes_midi, list) or len(chord_notes_midi) == 0:
            logger.warning(f"Invalid chord notes at index {i} for secondary rhythm, using C major")
            chord_notes_midi = CHORDS['C']
        ...
        for note in chord_notes_midi:
            ...
            rhythm_data.append((safe_note, time_pos_beats, chord_actual_duration, velocity))
...
def generate_bass_line_section(params, section_beats, current_chord_progression):
    """Generates bass line for a single section based on genre-specific style - FIXED"""
    bass_line_events = []
    time_pos_beats = 0.0
    
    # Validasi chord progression
    if not isinstance(current_chord_progression, list) or len(current_chord_progression) == 0:
        logger.warning("Invalid chord progression for bass, using C major")
        current_chord_progression = [CHORDS['C']]
    ...
    for i in range(len(current_chord_progression)):
        ...
        # Validasi chord notes
        if not isinstance(chord_notes_midi, list) or len(chord_notes_midi) == 0:
            logger.warning(f"Invalid chord notes at index {i} for bass, using C major")
            chord_notes_midi = CHORDS['C']
        ...
        if bass_style == 'walking': # Jazz/Blues
            for beat_idx in range(int(chord_actual_duration)):
                ...
                if current_sub_beat < section_beats:
                    ...
                    if beat_idx % 4 == 1: 
                        note_to_play += random.choice([2, 3, 4, 5, 7])
                    elif beat_idx % 4 == 2: 
                        note_to_play += random.choice([-1, 0, 1, 2, 3])
                    elif beat_idx % 4 == 3: 
                        note_to_play -= random.choice([0, 1, 2])
                    
                    # Validasi note
                    safe_note = max(24, min(72, int(note_to_play)))
                    velocity = random.randint(base_velocity - 10, base_velocity + 10)
                    bass_line_events.append((safe_note, current_sub_beat, 0.9, velocity))
                else: 
                    break
                ...
                if current_sub_beat < section_beats:
                    ...
                    if random.random() < 0.3:
                         if current_sub_beat + 0.5 < section_beats:
                            variation_note = root_note + random.choice([0, 5])
                            safe_note = max(24, min(72, int(variation_note)))
                            bass_line_events.append((safe_note, current_sub_beat + 0.5, 0.4, velocity - 10))
                else: 
                    break
            ...
            for beat_idx in range(int(chord_actual_duration / 1.0)):
                ...
                if current_sub_beat < section_beats:
                    ...
                    if random.random() < 0.7 and current_sub_beat + 0.5 < section_beats:
                        variation_note = root_note + random.choice([0, 7, 12])
                        safe_note = max(24, min(72, int(variation_note)))
                        bass_line_events.append((safe_note, current_sub_beat + 0.5, 0.4, base_velocity - 10))
                    if random.random() < 0.5 and current_sub_beat + 0.25 < section_beats:
                        safe_note = max(24, min(72, int(root_note + 12)))
                        bass_line_events.append((safe_note, current_sub_beat + 0.25, 0.2, base_velocity - 20))
                else: 
                    break
...
def generate_drum_pattern_section(params, section_type, section_beats):
    """Generates genre-specific drum pattern for a given section type - FIXED"""
    drum_events = []
    ...
    for current_beat_idx in range(0, int(section_beats)):
        ...
        if params['genre'] in ['rock', 'metal']:
            if current_beat_idx % 4 == 0:  # Beat 1 (double kick)
                drum_events.append((DRUM_NOTES['kick'], current_beat_time, 0.3, random.randint(kick_vel, 127)))
                drum_events.append((DRUM_NOTES['kick'], current_beat_time + 0.25, 0.3, random.randint(max(0, kick_vel-10), 127-10)))
            elif current_beat_idx % 4 == 2:  # Beat 3
                drum_events.append((DRUM_NOTES['kick'], current_beat_time, 0.5, random.randint(kick_vel, 127)))
        elif params['genre'] in ['hiphop', 'latin', 'dangdut']:
            # Syncopated/off-beat kicks
            drum_events.append((DRUM_NOTES['kick'], current_beat_time, 0.3, random.randint(kick_vel, kick_vel+10)))
            if random.random() < 0.4:  # Tambah off-beat kick
                drum_events.append((DRUM_NOTES['kick'], current_beat_time + 0.5, 0.2, random.randint(max(0, kick_vel-20), kick_vel)))
        else:  # Pop, Ballad, Blues, Jazz
            if current_beat_idx % 4 == 0 or current_beat_idx % 4 == 2:  # Beat 1 & 3
                drum_events.append((DRUM_NOTES['kick'], current_beat_time, 0.4, random.randint(kick_vel, kick_vel+10)))
        
        # Snare Drum (38)
        if params['genre'] in ['rock', 'metal']:
            if current_beat_idx % 4 == 1 or current_beat_idx % 4 == 3:  # Beat 2 & 4
                drum_events.append((DRUM_NOTES['snare'], current_beat_time, 0.4, random.randint(snare_vel, 127)))
        elif params['genre'] in ['hiphop', 'latin', 'dangdut']:
            if current_beat_idx % 4 == 1 or current_beat_idx % 4 == 3:  # Beat 2 & 4
                offset = random.choice([0, 0.125])
                drum_events.append((DRUM_NOTES['snare'], current_beat_time + offset, 0.3, random.randint(snare_vel, snare_vel+10)))
        else:  # Pop, Ballad, Blues, Jazz
            if current_beat_idx % 4 == 1 or current_beat_idx % 4 == 3:  # Beat 2 & 4
                drum_events.append((DRUM_NOTES['snare'], current_beat_time, 0.4, random.randint(snare_vel, snare_vel+10)))

        # Hi-hat (42) / Ride (51)
        if section_type == 'chorus' or params['genre'] in ['jazz', 'metal']:
            # Ride cymbal on eighth notes
            for eighth_sub_div in range(2):
                drum_events.append((DRUM_NOTES['ride'], current_beat_time + (eighth_sub_div * 0.5), 0.3, random.randint(max(0, ride_vel - 10), ride_vel + 10)))
            if section_type == 'chorus' and current_beat_idx % 2 == 0: # Crash setiap 2 beat di chorus
                drum_events.append((DRUM_NOTES['crash'], current_beat_time, 1.0, crash_vel)) # Loud crash
        else:
            # Hi-hat on 16th notes
            for sixteenth_sub_div in range(4):
                drum_events.append((DRUM_NOTES['hat_closed'], current_beat_time + (sixteenth_sub_div * 0.25), 0.1, random.randint(max(0, hat_vel - 10), hat_vel + 10)))
    ...
    if section_type == 'intro' and section_beats > 0:
        drum_events.append((DRUM_NOTES['crash'], 0, 1.0, crash_vel)) # Di awal section
    if section_type == 'outro' and section_beats > 1:
        drum_events.append((DRUM_NOTES['crash'], section_beats - 1, 1.0, crash_vel)) # Di akhir section
...
def build_song_structure(params):
    """Builds the song structure (intro, verse, chorus, etc.) and chord progressions for each section - FIXED"""
    ...
    # Validasi main chord progression
    chord_progression_main = params.get('chords', [])
    if not isinstance(chord_progression_main, list) or len(chord_progression_main) == 0:
        logger.warning("Invalid main chord progression, using C major")
        chord_progression_main = [CHORDS['C']]
        params['chords'] = chord_progression_main
    
    # Ambil chord progression bridge dari daftar, pastikan berbeda dari main
    try:
        genre_params = GENRE_PARAMS.get(params['genre'], GENRE_PARAMS['pop'])
        chord_progression_bridge_names = random.choice(genre_params['chord_progressions'])
        chord_progression_bridge = chord_names_to_midi_notes(chord_progression_bridge_names)
        
        # Pastikan berbeda dari main progression
        if (chord_progression_bridge == chord_progression_main and 
            len(genre_params['chord_progressions']) > 1):
            chord_progression_bridge_names = random.choice(genre_params['chord_progressions'])
            chord_progression_bridge = chord_names_to_midi_notes(chord_progression_bridge_names)
    except Exception as e:
        logger.warning(f"Error creating bridge progression: {e}, using main progression")
        chord_progression_bridge = chord_progression_main

    # Durasi dasar untuk setiap bagian
    base_beats = params.get('base_duration_beats_per_section', 
                           {'intro': 8, 'verse': 16, 'pre_chorus': 8, 'chorus': 16, 'bridge': 16, 'interlude': 8, 'outro': 8})
...
<head>
    ...
    <!-- Wavesurfer.js CDN dengan fallback -->
    <script src="https://unpkg.com/wavesurfer.js@7/dist/wavesurfer.min.js"></script>
    <script>
        // Fallback untuk Wavesurfer jika CDN utama gagal
        if (typeof Wavesurfer === 'undefined') {
            let script = document.createElement('script');
            script.src = 'https://cdn.jsdelivr.net/npm/wavesurfer.js@7/dist/wavesurfer.min.js';
            script.onerror = function() {
                console.error('All Wavesurfer CDNs failed. Audio visualization will be disabled.');
                window.wavesurferAvailable = false;
            };
            script.onload = function() {
                console.log('Wavesurfer loaded from fallback CDN');
                window.wavesurferAvailable = true;
            };
...
<body class="bg-gray-100 min-h-screen flex flex-col items-center justify-center p-4 font-sans antialiased">
    <div class="bg-white p-8 rounded-lg shadow-xl w-full max-w-2xl border border-gray-200">
        <h1 class="text-3xl font-extrabold text-center mb-6 text-gray-800">🎵 Generate Instrumental AI 🎵</h1>
        ...
        <form id="musicForm">
            ...
            <div class="mb-6">
                ...
                <input type="number" id="tempoInput" class="shadow-sm appearance-none border border-gray-300 rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out" placeholder="Auto (60-200)" min="60" max="200">
                <small class="text-gray-500">Kosongkan untuk auto-detect berdasarkan genre</small>
            </div>

            <!-- Tombol Generate -->
            <div class="flex items-center justify-center mb-6">
                <button type="submit" id="generateBtn" class="bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition duration-150 ease-in-out flex items-center justify-center px-6 py-3 text-white font-bold rounded-lg shadow-lg">
        ...
        <!-- Pesan Status -->
        <div id="statusMsg" class="text-center text-gray-600 text-sm mb-4 min-h-[1.5em] p-3 bg-gray-100 rounded-lg hidden"></div>
        ...
        <div id="audioSection" class="hidden">
            ...
            <div class="flex justify-center">
                <button id="downloadBtn" class="bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition duration-150 ease-in-out flex items-center justify-center px-6 py-3 text-white font-bold rounded-lg shadow-lg" disabled>
                    💾 Download MP3
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        let wavesurfer = null;
        let wavesurferAvailable = false;

        // Load Wavesurfer.js dengan fallback
        if (typeof Wavesurfer === 'undefined') {
        ...
        function initializeApp() {
            ...
            console.log('audioSection:', audioSection);
            console.log('audioPlayer:', audioPlayer);
            console.log('downloadBtn:', downloadBtn);
            console.log('genreSelect:', genreSelect);
            console.log('tempoInput:', tempoInput);
            console.log('waveformContainer:', waveformContainer);
            console.log('wavesurferAvailable:', window.wavesurferAvailable);
            ...
            musicForm.addEventListener('submit', async function(e) {
                ...
                document.getElementById('genreDisplay').textContent = 'N/A';
                document.getElementById('tempoDisplay').textContent = 'N/A';
                document.getElementById('durationDisplay').textContent = 'N/A';
                document.getElementById('lyricsWordCountDisplay').textContent = '0';
                document.getElementById('lyricsSyllableCountDisplay').textContent = '0';
                ...
                try {
                    ...
                    const timeoutId = setTimeout(() => {
                        ...
                        statusMsg.innerHTML = `
                            ...
                            <p class="text-xs text-gray-500 mt-1">
                                💡 Jika instrumental tetap tidak muncul, coba sesuaikan lirik atau periksa koneksi internet Anda.
                            </p>
                        `;
                    ...
                    if (response.ok) {
                        ...
                        if (data.success) {
                            ...
                            document.getElementById('tempoDisplay').textContent = `${data.tempo} BPM` || 'N/A';
                            document.getElementById('durationDisplay').textContent = `${data.duration} detik` || 'N/A';
                            document.getElementById('lyricsWordCountDisplay').textContent = data.size || '0';
                            document.getElementById('lyricsSyllableCountDisplay').textContent = data.soundfont || 'N/A';
                            
                            // Initialize Wavesurfer if available
                            if (window.wavesurferAvailable && typeof Wavesurfer !== 'undefined') {
                                ...
                                wavesurfer.on('error', (err) => {
                                    ...
                                    statusMsg.innerHTML += `<p class="text-red-600 mt-2">Error loading waveform: ${err.message}</p>`;
                                });
                                ...
                                audioPlayer.ontimeupdate = () => {
                                    if (!wavesurfer.isPlaying()) {
                                        wavesurfer.setTime(audioPlayer.currentTime);
                                    }
                                };
                                ...
                                console.warn('Wavesurfer not available. Audio visualization disabled.');
                                statusMsg.innerHTML += `<p class="text-yellow-600 mt-2">Audio visualization disabled. Wavesurfer.js not loaded.</p>`;
                            }
                        ...
                        statusMsg.innerHTML = `<p class="text-red-600">❌ Server Error: ${errorData.error || 'Gagal generate instrumental'}</p>`;
                    }
                } catch (error) {
                    generateBtn.disabled = false;
                    loadingSpinner.classList.add('hidden');
