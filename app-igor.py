...
# Import pyfluidsynth dengan error handling (opsional, untuk fallback)
try:
...
# Import pydub untuk manipulasi audio (WAW ke MP3, kompresi, dll)
from pydub import AudioSegment
from pydub.effects import normalize as pydub_normalize # Mengganti nama untuk menghindari konflik
...
def detect_genre_from_lyrics(lyrics):
    ...
    keywords = {
        ...
        'hiphop': ['rap', 'street', 'beat', 'flow', 'rhythm', 'hustle', 'city', 'time', 'crew'], # Fixed 'yme' to 'time'
...
def generate_harmony(params):
    ...
    for i, chord_notes in enumerate(chords):
        ...
        for j, note in enumerate(chord_notes):
            ...
            velocity = max(40, min(velocity, 100))
...
def create_midi_file(params, output_path):
    ...
    if harmony_instrument:
        ...
        for pitch, start_beat, duration_beat, velocity in harmony_data:
            ...
            final_velocity = min(127, max(40, int(velocity * 0.7)))
            if duration_ticks > 0:
    ...
    if params['drums_enabled']:
        ...
        if params['genre'] in ['pop', 'ballad', 'blues', 'jazz', 'hiphop']: # HipHop juga pakai pattern ini
    ...
    for track in mid.tracks:
        if len(track) == 0:
            # If track is completely empty, add an end_of_track message with time 0
            track.append(MetaMessage('end_of_track', time=0))
        elif not isinstance(track[-1], MetaMessage) or track[-1].type != 'end_of_track':
            # Calculate remaining delta time to the end of the full duration
            current_abs_time_at_end = sum(msg.time for msg in track)
            # Ensure end_of_track is precisely at the logical end of the piece
...
def midi_to_audio_subprocess(midi_path, output_wav_path, soundfont_path):
    ...
    try:
        cmd = [
            ...
            '-o', 'audio.file.endian=little', # ARM64 ENDIAN FIX
            '-o', 'audio.file.format=s16',
            '-o', 'synth.sample-rate=44100',
            '-a', 'null', # No real-time audio output
            '-ni', # No MIDI input
            '-g', '0.8', # Gain (0.8 = -2dB)
            str(soundfont_path),
            str(midi_path)
        ...
        logger.error("FluidSynth timeout (120s) - MIDI too complex or large SoundFont")
        return False
...
def wav_to_mp3(wav_path, mp3_path):
    """Convert WAV to MP3 with PROFESSIONAL audio processing and compression"""
    if not wav_path.exists() or wav_path.stat().st_size == 0:
        logger.error("Empty WAV file: {}".format(wav_path))
        return False

    try:
        logger.info("Converting WAV to MP3 with compression: {} -> {}".format(wav_path.name, mp3_path.name))
        ...
        # 1. NORMALIZATION
        logger.info("Step 1: Normalizing audio to -1dBFS peak...")
        audio = pydub_normalize(audio, headroom=1.0) # Using pydub_normalize from pydub.effects
        
        # 2. COMPRESSION
        logger.info("Step 2: Applying dynamic range compression (Threshold=-20dB, Ratio=4:1)...")
        ...
        # 3. EQ SIMULATION (Simplified)
        logger.info("Step 3: Applying EQ (Bass Boost +2dB, Clarity Boost +1dB)...")
        # Bass boost (low shelf)
        bass_boosted = compressed.low_pass_filter(250) + 2
        # High clarity boost (high shelf)
        eq_audio = bass_boosted.high_pass_filter(800) + 1 # Freq 800 for clarity

        # 4. LIMITING (to catch any peaks after EQ/Compression)
        logger.info("Step 4: Applying limiting to -0.5dBFS...")
        limited = eq_audio.apply_gain(-0.5) # Ensure no clipping beyond -0.5dBFS
        final_audio = limited
        
        # 5. FINAL GAIN STAGING (Target RMS for streaming platforms, e.g., -14 LUFS is approx -14dB RMS)
        logger.info("Step 5: Adjusting gain to target -14dB RMS for balanced loudness...")
        target_rms_db = -14.0 # Target RMS in dB for balanced loudness
        current_rms_db = final_audio.rms # RMS value from pydub is in dB
        ...
        # 6. FINAL CLIPPING PROTECTION
        if final_audio.max_dBFS > -0.1:
            logger.warning(f"Clipping detected after final gain staging! Max peak: {final_audio.max_dBFS:.1f}dBFS. Applying safety reduction.")
            final_audio = final_audio.apply_gain(-(final_audio.max_dBFS + 0.1)) # Bring down to -0.1dBFS
            
        # === HIGH-QUALITY MP3 EXPORT ===
        logger.info("Step 6: Exporting to 320kbps MP3 (Highest Quality)...")
        final_audio.export(
            mp3_path,
            format='mp3',
            bitrate='320k',        # High bitrate for quality
            parameters=['-q:a', '0'] # libmp3lame highest quality VBR. 'q:a' ensures audio quality option.
        )

        if mp3_path.exists() and mp3_path.stat().st_size > 1000:
            file_size = mp3_path.stat().st_size / 1024
            
            logger.info("🎵 PROFESSIONAL MP3 generated: {} ({:.1f} KB)".format(mp3_path.name, file_size))
            logger.info("📊 Audio Stats (after processing): Peak={:.1f}dBFS, RMS={:.1f}dB, Duration={:.1f}s".format(
                final_audio.max_dBFS, final_audio.rms, len(final_audio)/1000.0
            ))
            return True
        else:
            logger.warning("MP3 file too small or empty after conversion: {}".format(mp3_path))
            return False

    except Exception as e:
        logger.error("WAV to MP3 conversion error: {}".format(e), exc_info=True)
