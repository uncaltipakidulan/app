...
def generate_melody_section(params, section_beats, current_chord_progression, is_solo=False):
            ...
            for beat_sub_div in range(int(chord_actual_duration / 1.0)): # Every beat
                 ...
                 if current_sub_beat < section_beats:
                    for note in chord_notes_midi:
                        ...
                        rhythm_data.append((note, current_sub_beat, 0.9, velocity))
                 else: break
        ...
        if bass_style == 'walking': # Jazz/Blues
            ...
            for beat_sub_div in range(int(chord_actual_duration / 0.25)): # Every 16th note
                ...
                if current_sub_beat < section_beats:
                    ...
                    bass_line_events.append((root_note, current_sub_beat, 0.2, velocity)) # Short, staccato
                else: break
        
        elif bass_style == 'sustained': # Ballad
            velocity = random.randint(base_velocity - 20, base_velocity - 5)
            bass_line_events.append((root_note, time_pos_beats, chord_actual_duration * 0.9, velocity)) # Long sustain
...
def generate_drum_pattern_section(params, section_type, section_beats):
    ...
    for current_beat_idx in range(0, int(section_beats)):
        ...
        if random.random() < 0.05 and current_beat_idx % 4 == 3 and current_beat_idx < section_beats - 2: # Fill di akhir frase
            fill_time = current_beat_time + 0.5
            drum_events.append((47, fill_time, 0.2, tom_vel))
            drum_events.append((45, fill_time + 0.25, 0.2, tom_vel))
            drum_events.append((43, fill_time + 0.5, 0.2, tom_vel))

    # Crash Cymbal (49) - Intro/Outro/Chorus
    if section_type == 'intro' or section_type == 'chorus':
        if random.random() < 0.5 or section_type == 'intro': # Hanya di beberapa chorus
...
def build_song_structure(params):
    ...
    # --- INTRO ---
    song_structure.append(('intro', base_beats['intro'], chord_progression_main[:min(2, len(chord_progression_main))], False))
    ...
    remaining_beats = target_min_duration_beats - current_beats
    if remaining_beats > 0:
        outro_duration = max(base_beats['outro'], remaining_beats) # Set minimum 8 beats for outro
        song_structure.append(('outro', outro_duration, chord_progression_main[0:1], False))
        current_beats += outro_duration
    else: # If already long enough, just add a standard outro
        song_structure.append(('outro', base_beats['outro'], chord_progression_main[0:1], False))
...
def create_midi_file(params, output_path):
    ...
    for section_type, section_beats, chord_progression_for_section, is_solo_section in song_structure:
        ...
        # Melody
        melody_events, pb_events = generate_melody_section(params, section_beats, chord_progression_for_section, is_solo_section)
...
def main_app_runner():
    ...
    try:
        ...
        logger.info("Each genre has 5 chord progressions for variation!".format(len(next(iter(GENRE_PARAMS.values()))['chord_progressions']))) # FIXED: Ubah 10 menjadi 5 di sini karena GENRE_PARAMS memiliki 5 proggression sekarang
