...
def generate_harmony(params):
    ...
    # Velocity berdasarkan mood (dikurangi untuk mencegah bleed)
    if params['mood'] == 'sad' or params['mood'] == 'emotional':
        velocity = 50  # Revisi: Lebih rendah untuk harmony (piano sering di sini)
...
def create_midi_file(params, output_path):
    ...
    # MIDIFile(num_tracks, ticks_per_beat) - 4 tracks: melody (ch0), harmony (ch1), bass (ch2), drums (ch9)
    midi = MIDIFile(4, 120)  # Ticks_per_beat 120 adalah standar yang baik

    # Set tempo untuk semua track
    for i in range(4):
        midi.addTempo(i, 0, tempo)

    # Track 0: Melody (Channel 0 - paling keras, e.g., gitar distorsi)
    if params['instruments']['melody']:
        ...
        for pitch, time_pos, duration, velocity in melody_notes:
            final_velocity = min(127, int(velocity * 1.2))  # Cap di 127 untuk MIDI
            midi.addNote(0, 0, pitch, time_pos, duration, final_velocity)  # Channel 0
            # Revisi: Tambahkan explicit note off untuk mencegah bleed
            midi.addNoteOff(0, 0, pitch, time_pos + duration, 0)  # Note off dengan velocity 0

    # Track 1: Harmony/Chords (Channel 1 - e.g., piano, volume rendah untuk isolasi)
    if params['instruments']['harmony']:
        program_num = INSTRUMENTS.get(params['instruments']['harmony'], 48) # Default to String Ensemble 1
        midi.addProgramChange(1, 1, 0, program_num) # Track 1, Channel 1, Time 0, Program Number
        ...
        for pitch, start_beat, duration, velocity in harmony_data:
            final_velocity = min(127, int(velocity * 0.6))  # Revisi: Harmony 40% lebih rendah untuk kurangi bleed
            midi.addNote(1, 1, pitch, start_beat, duration, final_velocity)  # Channel 1
            # Revisi: Explicit note off
    ...
    if params['instruments']['bass']:
        ...
        midi.addProgramChange(2, 2, 0, program_num) # Track 2, Channel 2, Time 0, Program Number
        ...
        for pitch, start_beat, duration, velocity in bass_notes:
            final_velocity = min(127, int(velocity * 1.1))  # Cap di 127
            midi.addNote(2, 2, pitch, start_beat, duration, final_velocity)  # Channel 2
            # Revisi: Explicit note off
            midi.addNoteOff(2, 2, pitch, start_beat + duration, 0)

    # Track 3: Drums (Channel 9 - standar MIDI percussion)
    if params['drums_enabled']: # Gunakan drums_enabled
        ...
        for beat_pos in range(int(duration_beats)):
            ...
            if beat_pos % 4 == 0 or beat_pos % 4 == 2:
                midi.addNote(3, 9, 36, beat_pos, 0.5, 110)  # Bass drum (MIDI note 36)
            # Snare on beats 2 dan 4
        ...
        # Drums tidak butuh note off karena percussion bersifat one-shot
...
def midi_to_audio_subprocess(midi_path, output_wav_path, soundfont_path):
    ...
    try:
        cmd = [
            ...
            '-g', '0.8',    # Revisi: Gain sedikit lebih rendah untuk instrumen berat seperti distorsi
            '-R', '0',      # Revisi: Reverb off untuk mencegah bleed
            '-E', '0',      # Revisi: Chorus off untuk isolasi suara
            '-l',           # Loop mode off
            str(soundfont_path),
            str(midi_path)
        ]
    ...
    <head>
        ...
        <style>
            body {
                  ...
                  color: #333;
                  line-height: 1.6; /* Tambahkan untuk readability */
                  }

/* Optional: Tambahkan media query untuk mobile */
@media (max-width: 600 px) {
    body {
        ...
        max-width: 100%;
            ...
            h1 { color: #2c3e50; text-align: center; margin-bottom: 30 px; }
            p { text-align: center; color: #555; margin-bottom: 25 px; }
            form { background: #fff; padding: 30 px; border-radius: 8 px; box-shadow: 0 4 px 10 px rgba(0,0,0,0.05); }
            label { display: block; margin-bottom: 8 px; font-weight: bold; color: #34495e; }
            textarea { width: calc(100% - 22 px); height: 180 px; margin-bottom: 20 px; padding: 10 px; border: 1 px solid #ddd; border-radius: 5 px; font-size: 16 px; resize: vertical; }
            select, input[type="number"] { width: calc(100% - 22 px); padding: 10 px; margin-bottom: 20 px; border: 1 px solid #ddd; border-radius: 5 px; font-size: 16 px; background-color: #f9f9f9; }
            input[type="number"] { width: 150 px; display: inline-block; margin-right: 10 px; }
            small { color: #7f8c8d; font-size: 0.9 em; }
            ...
            .result { margin-top: 30 px; padding: 25 px; background: #e8f6f3; border-radius: 8 px; border: 1 px solid #d1eeea; text-align: center; }
            .result h3 { color: #2c3e50; margin-bottom: 15 px; }
            .result p { margin: 8 px 0; color: #444; }
            audio { width: 100%; margin: 20 px 0; border-radius: 5 px; }
    ...
    <body>
        <h1>(U+F3B5) Generate Instrumental (U+F3B5)</h1>
