...
GENRE_PARAMS = {
    'pop': {
        ...
        'instruments': {
            ...
            # SOLUSI: Mengganti instrumen yang "basah" dengan yang lebih "kering"
            'rhythm_secondary': 'Electric Piano 1', # Sebelumnya 'String Ensemble 1'
...
def create_midi_file(params, output_path):
    try:
        ...
        tracks = {name: MidiTrack() for name in ['melody', 'rhythm_primary', 'rhythm_secondary', 'bass', 'drums']}
        ...
        tracks['rhythm_primary'].append(Message('control_change', channel=1, control=7, value=80, time=0)) # Volume
        # SOLUSI: Matikan efek Reverb, Chorus, dan perpendek Release Time
        tracks['rhythm_primary'].append(Message('control_change', channel=1, control=91, value=0, time=0))   # Reverb Level
        tracks['rhythm_primary'].append(Message('control_change', channel=1, control=93, value=0, time=0))   # Chorus Level
        tracks['rhythm_primary'].append(Message('control_change', channel=1, control=72, value=20, time=0))  # Release Time (0-127)
        ...
        tracks['rhythm_secondary'].append(Message('control_change', channel=2, control=7, value=70, time=0)) # Volume
        # SOLUSI: Matikan efek Reverb, Chorus, dan perpendek Release Time
        tracks['rhythm_secondary'].append(Message('control_change', channel=2, control=91, value=0, time=0))   # Reverb Level
        tracks['rhythm_secondary'].append(Message('control_change', channel=2, control=93, value=0, time=0))   # Chorus Level
        tracks['rhythm_secondary'].append(Message('control_change', channel=2, control=72, value=20, time=0))  # Release Time (0-127)

        # Bass (Ch 3)
        prog = INSTRUMENTS.get(params['instruments']['bass'], 0)
        tracks['bass'].append(Message('program_change', channel=3, program=prog, time=0))
        tracks['bass'].append(Message('control_change', channel=3, control=7, value=110, time=0)) # Volume

        # --- Proses Pembuatan Lagu per Bagian ---
        song_structure = build_song_structure(params)
        current_beat, all_events, tempo_events = 0.0, {ch: [] for ch in [0, 1, 2, 3, 9]}, []
        gen_funcs = {0: generate_melody_section, 1: generate_rhythm_primary_section,
...
def generate_instrumental_endpoint():
    ...
    try:
        ...
        if len(lyrics) < 10: return jsonify({'error': 'Lirik minimal 10 karakter.'}), 400
