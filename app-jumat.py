...
def generate_drum_pattern_section(params, section_type, section_beats):
    ...
    if section_type == 'intro' or section_type == 'ballad' and section_type != 'chorus':
...
def build_song_structure(params):
    ...
    structure = [
        ('intro', 8, prog[:2]),
        ('verse', 16, prog),
        ('pre_chorus', 8, prog[-2:] + prog[:2]),
        ('chorus', 16, prog),
        ('verse', 16, prog),
        ('chorus', 16, prog),
        ('bridge', 8, prog[1:] + prog[:1]),
        ('chorus', 16, prog),
        ('outro', 8, [prog[-1]])
    ]
    ...
    try:
        ...
        for event in master_events:
            ...
            if event_type == 'tempo':
                ...
                target_track.append(MetaMessage('set_tempo', tempo=tempo_val, time=delta_tick))
                ...
                target_track.append(Message('note_on', channel=ch, note=int(pitch), velocity=int(vel), time=delta_on))
                last_tick_on_track[track_name] = abs_tick

                # Siapkan Note Off
                off_beat = abs_beat + dur
                off_tick = beats_to_ticks(off_beat)
                all_events[ch].append(('note_off', off_beat, ch, pitch))

        # Proses semua Note Off
        all_note_offs = []
        for ch in all_events:
            for event in all_events[ch]:
                if event[0] == 'note_off':
                    all_note_offs.append(event)
        all_note_offs.sort(key=lambda x: x[1])

        for _, off_beat, ch, pitch in all_note_offs:
            off_tick = beats_to_ticks(off_beat)
            track_name = track_map[ch]
            target_track = tracks[track_name]
            delta_off = off_tick - last_tick_on_track[track_name]
            target_track.append(Message('note_off', channel=ch, note=int(pitch), velocity=0, time=delta_off))
            last_tick_on_track[track_name] = off_tick

        # Tambahkan End of Track
        total_ticks = beats_to_ticks(current_absolute_beat + 2)
