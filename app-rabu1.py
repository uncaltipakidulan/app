...
GENRE_PARAMS = {
    'pop': {
        ...
        'instruments': {
            'melody': ['Clean Electric Guitar', 'Flute', 'Overdriven Guitar', 'Soprano Sax'], # Pilihan melody
            'rhythm_primary': ['Electric Piano 1', 'Clean Electric Guitar'], # Piano untuk rhythm
    ...
    'rock': {
        ...
        'instruments': {
            'melody': ['Distortion Guitar', 'Rock Organ', 'Overdriven Guitar'], # Gitar distorsi untuk melody
    ...
    'metal': {
        ...
        'instruments': {
            'melody': ['Distortion Guitar', 'Orchestral Strings', 'Overdriven Guitar'], # Gitar distorsi
            'rhythm_primary': ['Power Chord'], # Power chord
...
def generate_melody_section(params, section_beats, current_chord_progression, is_solo=False, add_expressive_effects=True):
    ...
    while time_pos_beats < section_beats:
        ...
        current_chord_name = current_chord_progression[chord_index]
        
        # FIXED: Convert chord name to MIDI notes if it's a string
        if isinstance(current_chord_name, str):
            current_chord_notes = CHORDS.get(current_chord_name, CHORDS['C'])
        else:
            current_chord_notes = current_chord_name
        
        possible_pitches = [p for p in scale_notes if p % 12 in [n % 12 for n in current_chord_notes]]
...
def generate_rhythm_primary_section(params, section_beats, current_chord_progression):
    ...
    for i in range(len(current_chord_progression)):
        chord_name = current_chord_progression[i]
        # FIXED: Convert chord name to MIDI notes if it's a string
        if isinstance(chord_name, str):
            chord_notes_midi = CHORDS.get(chord_name, CHORDS['C'])
        else:
            chord_notes_midi = chord_name
    ...
    for current_beat_idx in range(0, int(section_beats)):
        ...
        if params['genre'] in ['rock', 'metal']:
            if current_beat_idx % 4 == 0:  # Beat 1 (double kick)
                ...
                drum_events.append((DRUM_NOTES['kick'], current_beat_time + 0.25, 0.3, random.randint(kick_vel-10, 127-10)))
            ...
            if random.random() < 0.4:  # Tambah off-beat kick
                drum_events.append((DRUM_NOTES['kick'], current_beat_time + 0.5, 0.2, random.randint(kick_vel-20, kick_vel)))
        ...
        if params['genre'] in ['rock', 'metal']:
            ...
            if current_beat_idx % 4 == 1 or current_beat_idx % 4 == 3:  # Beat 2 & 4
                drum_events.append((DRUM_NOTES['snare'], current_beat_time + random.choice([0, 0.125]), 0.3, random.randint(snare_vel, snare_vel+10))) # Sedikit syncopated
        else:  # Pop, Ballad, Blues, Jazz
            if current_beat_idx % 4 == 1 or current_beat_idx % 4 == 3:  # Beat 2 & 4
        ...
        if section_type == 'chorus' or params['genre'] in ['jazz', 'metal']:
            ...
            for eighth_sub_div in range(2):
                drum_events.append((DRUM_NOTES['ride'], current_beat_time + (eighth_sub_div * 0.5), 0.3, random.randint(ride_vel - 10, ride_vel + 10)))
            ...
            for sixteenth_sub_div in range(4):
                drum_events.append((DRUM_NOTES['hat_closed'], current_beat_time + (sixteenth_sub_div * 0.25), 0.1, random.randint(hat_vel - 10, hat_vel + 10)))
...
def build_song_structure(params):
    ...
    chord_progression_main = params['selected_progression']  # Use the selected progression names
    ...
    if not intro_progression: # Fallback if main progression is too short
        intro_progression = params['selected_progression'][:min(len(params['selected_progression']), 2)]
        if not intro_progression: intro_progression = ['C'] # Final fallback
...
def create_midi_file(params, output_path):
    ...
    bass_track.append(Message('control_change', channel=2, control=10, value=64, time=0))  # Pan Center
    ...
    <div class="bg-white p-8 rounded-lg shadow-xl w-full max-w-2xl border border-gray-200">
        ...
        <form id="musicForm">
            ...
            <div class="mb-6">
                ...
                <small class="text-gray-500">Kosongkan bila ingin tempo otomatis.</small>
            </div>
    ...
    <script>
        ...
        function initializeApp() {
            ...
            musicForm.addEventListener('submit', async function(e) {
                ...
                // Reset UI
                statusMsg.textContent = '🚀 Memulai generate instrumental...';
