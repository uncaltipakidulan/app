...
def wav_to_mp3(wav_path, mp3_path):
    ...
    try:
        ...
        # Jika Anda ingin mengikuti standar loudness seperti -14 LUFS (sekitar -14dB RMS),
        # langkah ini bisa diaktifkan. Untuk saat ini kita cukup mengandalkan limiter.
        # Atau jika Anda benar-benar ingin target RMS tertentu:
        # target_rms_db = -12.0 # Misalnya target RMS -12dBFS
        # current_rms_db = limited.rms # pydub's rms is not dBFS. Use limited.dBFS to estimate.
        # if limited.dBFS > -10: # Only if current audio is loud enough to target
        #     gain_needed = target_rms_db - limited.dBFS # Ini masih perkiraan, bukan RMS yang akurat.
        #     final_audio = limited + gain_needed
        # else:
        #     final_audio = limited
        final_audio = limited # Untuk saat ini, limiter cukup.

        # 7. FINAL SANITY CHECK FOR CLIPPING
        if final_audio.max_dBFS > -0.1: # Jika masih ada puncak yang sangat dekat 0dBFS
            logger.warning(f"Clipping detected after final gain! Max peak: {final_audio.max_dBFS:.1f}dBFS. Applying safety reduction.")
