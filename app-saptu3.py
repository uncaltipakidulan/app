...
# Perbaikan: Hanya import normalize, compress_dynamic_range tidak tersedia
...
def get_music_params_from_lyrics(genre, lyrics, user_tempo_input='auto'):
    ...
    if genre not in GENRE_PARAMS:
        logger.warning(f"Genre '{genre}' tidak dikenali, menggunakan default 'pop'")
        genre = 'pop'
    ...
    if user_tempo_input != 'auto':
        try: 
            params['tempo'] = max(60, min(200, int(user_tempo_input)))
...
def create_midi_file(params, output_path):
    try:
        ...
        for i, (stype, sbeats, cprog) in enumerate(song_structure):
...
def check_dependencies():
    """Cek apakah semua dependency terinstall dengan benar"""
    dependencies_ok = True
    
    # Cek fluidsynth
    try:
        subprocess.run(['fluidsynth', '--version'], capture_output=True, check=True, timeout=10)
        logger.info("✓ FluidSynth terdeteksi")
    except (subprocess.CalledProcessError, FileNotFoundError, subprocess.TimeoutExpired):
        logger.error("✗ FluidSynth tidak terinstall atau tidak ada di PATH")
        dependencies_ok = False
    
    # Cek ffmpeg untuk pydub
    try:
        from pydub.utils import which
        if which("ffmpeg"):
            logger.info("✓ FFmpeg terdeteksi")
        else:
            logger.error("✗ FFmpeg tidak terinstall")
            dependencies_ok = False
    except ImportError:
        logger.error("✗ Pydub tidak terinstall dengan benar")
        dependencies_ok = False
    
    # Cek soundfont
    if SOUNDFONT_PATH:
        logger.info(f"✓ SoundFont ditemukan: {SOUNDFONT_PATH.name}")
    else:
        logger.warning("⚠ SoundFont tidak ditemukan")
    
    return dependencies_ok

def midi_to_audio(midi_path, output_wav_path):
    if not SOUNDFONT_PATH: 
        logger.error("SoundFont tidak tersedia")
        return False
    try:
        cmd = ['fluidsynth', '-F', str(output_wav_path), '-g', '1.0', '-ni', str(SOUNDFONT_PATH), str(midi_path)]
        logger.info("Rendering MIDI dengan FluidSynth...")
        ...
        logger.error("FluidSynth timeout setelah 120 detik")
        return False
    except Exception as e:
...
def wav_to_mp3(wav_path, mp3_path):
    try:
        if not wav_path.exists():
            logger.error(f"File WAV tidak ditemukan: {wav_path}")
            return False
        
        audio = AudioSegment.from_wav(wav_path)
        
        # Kompresi dinamis manual sebagai alternatif
        try:
            # Reduce dynamic range secara manual
            compressed_audio = audio.apply_gain(-3)  # Sedikit reduce gain
            logger.info("Menerapkan kompresi dinamis sederhana")
        except Exception as comp_error:
            logger.warning(f"Kompresi gagal, menggunakan audio asli: {comp_error}")
...
def cleanup_old_files(directory, max_age_hours=24):
    """Hapus file audio lama untuk menghemat space"""
    try:
        cutoff = datetime.now() - timedelta(hours=max_age_hours)
        for f in Path(directory).glob("*"):
            if f.suffix.lower() in ['.mp3', '.wav', '.mid']:
...
def index():
    ...
    <head> 
        ...
        <title>🎵 AI Instrumental Generator 🎵</title> 
    ...
    <body class="bg-gray-100 min-h-screen flex items-center justify-center p-4"> 
        <div class="bg-white p-8 rounded-lg shadow-xl w-full max-w-2xl"> 
            <h1 class="text-3xl font-bold text-center mb-6 text-gray-800">🎵 AI Instrumental Generator 🎵</h1> 
            <form id="musicForm"> 
                <div class="mb-4"> 
                    <label for="textInput" class="block text-gray-700 text-sm font-bold mb-2">Lirik / Deskripsi:</label> 
                    <textarea id="textInput" name="lyrics" rows="6" class="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Masukkan lirik atau deskripsi singkat..."></textarea> 
                </div> 
                <div class="grid grid-cols-2 gap-4 mb-6"> 
                    <div> 
                        <label for="genreSelect" class="block text-gray-700 text-sm font-bold mb-2">Genre:</label> 
                        <select id="genreSelect" name="genre" class="w-full p-3 border border-gray-300 rounded-lg"> 
                    ...
                    <div> 
                        ...
                        <input type="number" id="tempoInput" name="tempo" class="w-full p-3 border border-gray-300 rounded-lg" placeholder="Auto (60-200)" min="60" max="200"> 
                    </div> 
                </div> 
                <button type="submit" id="generateBtn" class="w-full bg-blue-600 text-white py-3 px-4 rounded-lg font-bold hover:bg-blue-700 transition">🚀 Generate Instrumental</button> 
            </form> 
            
            <div id="loading" class="loading mt-4 text-center">
                <div class="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
                <p class="mt-2 text-gray-600" id="loadingText">Mempersiapkan...</p>
            </div>
            
            <div id="status" class="text-center mt-4"></div> 
            <div id="audioSection" class="hidden mt-6"> 
                <h2 class="text-xl font-bold text-gray-800 mb-2">Hasil Instrumental</h2> 
                <div id="waveform" class="mb-2"></div> 
                <audio id="audioPlayer" class="w-full" controls></audio> 
                <a id="downloadBtn" class="w-full block text-center bg-green-600 text-white mt-4 py-3 px-4 rounded-lg font-bold hover:bg-green-700 transition" href="#" download>💾 Download MP3</a> 
            </div> 
        </div> 
        <script> 
            let wavesurfer = null; 
            document.getElementById('musicForm').addEventListener('submit', async (e) => { 
                ...
                try { 
                    loadingText.textContent = 'Menganalisis lirik dan membuat struktur lagu...';
                    ...
                    if (response.ok && data.success) { 
                        status.innerHTML = `<p class="text-green-600">🎉 Berhasil! Genre: <strong>${data.genre}</strong>, Tempo: <strong>${data.tempo} BPM</strong></p>`; 
                        audioSection.classList.remove('hidden'); 
                        const audioPlayer = document.getElementById('audioPlayer'); 
                        const downloadBtn = document.getElementById('downloadBtn'); 
                        const audioUrl = `/static/audio_output/${data.filename}`; 
                        audioPlayer.src = audioUrl; 
                        downloadBtn.href = audioUrl; 
                        downloadBtn.download = `instrumental_${data.id}.mp3`; 
...
def generate_instrumental_endpoint():
    ...
    try:
        ...
        if genre == 'auto': 
            genre = detect_genre_from_lyrics(lyrics)
        ...
        logger.info(f"Memulai generasi instrumental untuk ID: {unique_id}")
        
        if not create_midi_file(params, paths['mid']): 
            raise Exception("Gagal membuat file MIDI.")
        
        if not midi_to_audio(paths['mid'], paths['wav']): 
        ...
        try:
            ...
            logger.warning(f"Gagal menghapus file sementara: {cleanup_error}")

        logger.info(f"Generation complete in {time.time() - start_time:.1f}s!")
        return jsonify({
            'success': True, 
            'filename': paths['mp3'].name, 
            'genre': genre.capitalize(),
            'tempo': params['tempo'], 
            'id': unique_id,
            'progression': params.get('selected_progression', [])
        })
    except Exception as e:
...
def health_check():
    ...
    return jsonify({
        ...
        'dependencies': {
            'fluidsynth': 'ok' if dependencies_ok else 'missing',
            'ffmpeg': 'ok' if dependencies_ok else 'missing',
            'soundfont': 'found' if SOUNDFONT_PATH else 'missing'
        },
