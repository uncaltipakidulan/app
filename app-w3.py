...
# Konfigurasi logging - FIXED
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler('app.log'),
        logging.StreamHandler(sys.stdout) # FIXED: Gunakan StreamHandler untuk sys.stdout
    ]
)
        ...
        if bass_style == 'walking': # Jazz/Blues
            for beat_idx in range(int(chord_actual_duration)):
                ...
                if current_sub_beat < section_beats:
                    ...
                    note_to_play = max(24, min(note_to_play, 72)) # Clamp note to a reasonable range
...
def generate_drum_pattern_section(params, section_type, section_beats):
    ...
    for note, time_pos, duration, vel in drum_events:
        ...
        safe_vel = max(0, min(127, int(round(vel))))   # Ensure integer 0-127
        safe_time = float(round(time_pos, 3))          # Round time to avoid float precision issues
        safe_dur = max(0.01, float(round(duration, 3)))  # Minimum duration, no upper clamp as it's typically tied to beat
        
        # Mido doesn't like velocity 0 for note_on, it's typically note_off
        if safe_vel == 0: 
            safe_vel = 1 # Set to minimum audible if somehow 0
