...
from midiutil import MIDIFile # Pastikan ini adalah versi terbaru yang mendukung ticks_per_beat
import subprocess
import tempfile
from textblob import TextBlob
# import pyphen # Pyphen tidak digunakan di kode yang ada, jadi di-komentari atau bisa dihapus
...
def check_python_dependencies():
    ...
    deps = {
        ...
        # 'Pyphen': check_module('pyphen'), # Pyphen tidak digunakan
...
# Check MIDIUtil version and compatibility
def check_midiutil_version():
    """Check if MIDIUtil supports ticks_per_beat parameter"""
    try:
        # Test if ticks_per_beat is supported
        test_midi = MIDIFile(1, ticks_per_beat=480)
        logger.info("MIDIUtil version supports ticks_per_beat parameter")
        return True
    except TypeError:
        logger.warning("MIDIUtil version does NOT support ticks_per_beat. Using default resolution.")
        return False
    except Exception as e:
        logger.error(f"Error checking MIDIUtil version: {e}")
        return False

MIDIUTIL_SUPPORTS_TICKS = check_midiutil_version()
...
GENRE_PARAMS = {
    ...
    'jazz': {
        ...
        'instruments': {
            ...
            'bass': ['Electric Bass Bass', 'Acoustic Bass'],
        },
        'drums_enabled': True,
...
def create_midi_file(params, output_path):
    """Create multi-track MIDI file with channel isolation and expressive effects - FIXED VERSION"""
    tempo = params['tempo']
    duration_beats = params['duration_beats']

    # 4 tracks: Melody(0), Harmony(1), Bass(2), Drums(3)
    # CRITICAL: Gunakan ticks_per_beat secara kondisional
    if MIDIUTIL_SUPPORTS_TICKS:
        midi = MIDIFile(4, ticks_per_beat=960) # Increased ticks_per_beat for finer resolution
    else:
        midi = MIDIFile(4) # Fallback to default ticks_per_beat if not supported
        logger.warning("MIDIFile initialized without custom ticks_per_beat. Consider updating midiutil.")
    ...
    if melody_instrument:
        ...
        # Generate melody dengan pitch bend events yang sudah dibersihkan
        melody_events, pitch_bend_events = generate_melody(params, add_expressive_effects=True)

        # Set controller events - CRITICAL untuk pitch bend functionality
        # midiutil's addControllerEvent uses time as 'beat' (float)
        controller_start_time = -0.5 # A small negative value or 0.0
        # NOTE: addControllerEvent is expected to handle float time correctly,
        # but the value should be quantized to MIDI ticks internally.
        midi.addControllerEvent(0, 0, 0, controller_start_time, 0)   # Bank select MSB
        midi.addControllerEvent(0, 0, 100, controller_start_time, 0) # RPN MSB = 0 (Pitch Bend Range)
        midi.addControllerEvent(0, 0, 101, controller_start_time, 0) # RPN LSB = 0
        midi.addControllerEvent(0, 0, 6, controller_start_time, 2)   # Data Entry MSB = 2 semitones
        midi.addControllerEvent(0, 0, 38, controller_start_time, 0)  # Data Entry LSB = 0
        midi.addControllerEvent(0, 0, 7, controller_start_time, 100) # Volume
        midi.addControllerEvent(0, 0, 10, controller_start_time, 64) # Pan center
        
        # Tambahkan notes - QUANTIZE waktu ke 16th notes untuk konsistensi
        for pitch, time_pos, duration, velocity in melody_events:
            # Quantize start time ke 32nd notes (0.03125 beats) untuk resolusi lebih halus
            # Jika ticks_per_beat default (120), resolusi 0.03125 tidak akan optimal, tapi tetap dicoba
            quantized_time = round(time_pos / 0.03125) * 0.03125
            quantized_duration = round(duration / 0.03125) * 0.03125
            
            # Pastikan durasi minimum dan maksimum
            quantized_duration = max(0.03125, min(quantized_duration, 4.0)) # Min 32nd note
            
            final_velocity = min(127, max(40, int(velocity * 1.2)))  # Boost melody
            midi.addNote(0, 0, pitch, quantized_time, quantized_duration, final_velocity)
        
        # Tambahkan pitch bend events - CRITICAL: SEMUA INTEGER DAN QUANTIZED
        previous_time = -1.0 # Initialize to ensure first event is added
        # Gunakan resolusi ticks_per_beat actual jika tersedia, jika tidak pakai default 120
        # Default 480 atau 120. Kita pakai 120 (default lama) untuk min_time_diff_for_bend
        actual_ticks_per_beat = midi.ticks_per_beat if hasattr(midi, 'ticks_per_beat') else 120
        min_time_diff_for_bend = 1 / actual_ticks_per_beat # Minimum time difference in beats

        for event_time, bend_value in pitch_bend_events:
            # Quantize time lagi untuk konsistensi (32nd notes)
            # Ini akan mengonversi waktu float ke tick MIDI internal.
            quantized_time = round(event_time / 0.03125) * 0.03125
            
            # CRITICAL: Pastikan bend_value adalah integer dan dalam range yang valid
            safe_bend = max(-8192, min(8191, int(round(bend_value))))
            
            # Skip jika waktu terlalu dekat dengan sebelumnya (min 1 tick difference)
            if quantized_time >= previous_time + min_time_diff_for_bend:
                try:
                    # addPitchWheelEvent juga akan mengonversi waktu ke internal tick.
                    # Pastikan bend_value adalah int di sini.
                    midi.addPitchWheelEvent(0, 0, safe_bend, quantized_time)
                    previous_time = quantized_time
                    logger.debug(f"Added pitch bend: {safe_bend} at {quantized_time}")
                except Exception as e:
                    logger.warning(f"Skipping pitch bend at {quantized_time} with value {safe_bend}: {e}")
        
        # Reset pitch bend ke 0 di akhir track
        try:
            midi.addPitchWheelEvent(0, 0, 0, duration_beats)
        except Exception as e:
            logger.warning(f"Could not add final pitch reset: {e}")
    ...
    if harmony_instrument:
        ...
        midi.addControllerEvent(1, 1, 7, 80, 0)  # Volume
        midi.addControllerEvent(1, 1, 10, 64, 0)  # Pan
        ...
        for pitch, start_beat, duration, velocity in harmony_data:
            # Harmony quantized ke quarter notes (0.25 beats)
            quantized_time = round(start_beat / 0.25) * 0.25
            quantized_duration = round(duration / 0.25) * 0.25
            
            final_velocity = min(127, max(40, int(velocity * 0.6)))  # Softer harmony
            if quantized_duration > 0.01:
                midi.addNote(1, 1, pitch, quantized_time, quantized_duration, final_velocity)
    ...
    if bass_instrument:
        ...
        midi.addControllerEvent(2, 2, 7, 110, 0)  # Louder bass
        midi.addControllerEvent(2, 2, 10, 64, 0)  # Pan
        ...
        for pitch, start_beat, duration, velocity in bass_notes:
            # Bass quantized ke 8th notes (0.125 beats)
            quantized_time = round(start_beat / 0.125) * 0.125
            quantized_duration = round(duration / 0.125) * 0.125
            
            final_velocity = min(127, max(40, int(velocity * 1.1)))  # Strong bass
            if quantized_duration > 0.01:
                midi.addNote(2, 2, pitch, quantized_time, quantized_duration, final_velocity)

    # Track 3: Drums (Channel 9) - Standard GM Kit
    if params['drums_enabled']:
        # Drums tidak perlu program change (channel 9 adalah percussion)
        midi.addControllerEvent(3, 9, 7, 100, 0)  # Drum volume
        logger.info("Drums track: Standard GM Kit (Channel 9)")

        # Enhanced drum pattern dengan quantization - SEMUA INTEGER TIMES
        for beat_pos in range(0, int(duration_beats), 1):  # Per beat
            beat_time = float(beat_pos)
            
            # Kick drum (36) on beats 1 & 3
            if beat_pos % 4 == 0 or beat_pos % 4 == 2:
                midi.addNote(3, 9, 36, beat_time, 0.25, 110)  # Shorter kick
                # Sidechain effect: ghost notes
                if beat_pos % 8 == 4:  # Tambahan kick pattern
                    midi.addNote(3, 9, 36, beat_time + 0.5, 0.125, 70)

            # Snare drum (38) on beats 2 & 4
            if beat_pos % 4 == 1 or beat_pos % 4 == 3:
                midi.addNote(3, 9, 38, beat_time, 0.25, 95)
                # Snare fill
                if beat_pos % 8 == 7:
                    midi.addNote(3, 9, 38, beat_time + 0.25, 0.125, 80)

            # Hi-hat (42) every 8th note untuk groove yang lebih halus
            for eighth in range(2):
                hat_time = beat_time + (eighth * 0.125)
                hat_velocity = 75 if eighth == 0 else 60  # Stronger on beat
                midi.addNote(3, 9, 42, hat_time, 0.125, hat_velocity)

            # Additional percussion untuk genre-specific
            if params['genre'] in ['latin', 'dangdut'] and beat_pos % 2 == 0:
                # Conga atau shaker pattern
                perc_time = beat_time + 0.125
                perc_note = random.choice([43, 45, 49])  # Tom/Conga variations
                midi.addNote(3, 9, perc_note, perc_time, 0.125, 60)

        # Tambahkan crash cymbal di awal dan akhir
        midi.addNote(3, 9, 49, 0, 0.5, 100)  # Intro crash
        if duration_beats > 1:
            midi.addNote(3, 9, 49, duration_beats - 1, 0.5, 90)  # Outro crash

    # FINAL SAFETY CHECK sebelum write - CRITICAL FIX
    # --- START OF AGGRESSIVE PITCH BEND CLEANUP (ULTIMATE FIX) ---
    # Ini adalah upaya paling terakhir untuk memastikan semua pitch_wheel_value adalah int
    # sebelum midiutil mencoba menulisnya.
    try:
        # NOTE: Karena log menyatakan 'MIDITrack' object has no attribute 'events',
        # pendekatan iterasi midi.tracks[track_idx].events ini akan menyebabkan error
        # jika versi midiutil yang digunakan memang tidak mengeksposnya.
        # Jika memang gagal di sini, kita terpaksa harus berasumsi bahwa
        # addPitchWheelEvent sudah benar-benar menyimpan integer.
        
        # Saya akan menghapus loop aggressive cleanup yang lama karena terbukti gagal
        # dan menghasilkan 'MIDITrack' object has no attribute 'events'

        # Sebagai gantinya, saya akan menambahkan log yang lebih spesifik jika error ini
        # terjadi lagi, untuk memahami *di mana* float itu berasal.
        pass

    except Exception as e:
        logger.error(f"Error during aggressive pitch bend cleanup loop: {e}", exc_info=True)
    # --- END OF AGGRESSIVE PITCH BEND CLEANUP ---

    try:
        logger.info("Validating MIDI structure before writing...")
        
        # Test MIDI structure dengan temporary file
        temp_path = output_path.with_suffix('.tmp.mid')
        with open(temp_path, 'wb') as temp_f:
            midi.writeFile(temp_f) # Ini adalah baris yang memicu error
        
        # Jika berhasil, rename ke file final
        if temp_path.exists():
            temp_path.rename(output_path)
            logger.info("MIDI generated successfully with channel isolation: {}".format(output_path.name))
            return True
            
    except Exception as write_error:
        logger.error("Error writing MIDI file: {}".format(write_error), exc_info=True)
        
        # Debug: Print event types untuk troubleshooting
        try:
            logger.error(f"CRITICAL: MIDI write failed. Attempting to inspect MIDI object for float pitch bends.")
            # Ini adalah bagian debug tambahan untuk mencoba menemukan float
            # Saya akan mencoba mengakses atribut internal _tracks dari MIDIFile jika ada
            if hasattr(midi, '_tracks'): # Coba akses atribut internal
                for track_idx, track_data in enumerate(midi._tracks):
                    logger.debug(f"Inspecting internal track data for track {track_idx}")
                    if isinstance(track_data, list): # _tracks bisa jadi list event lists
                        for event_item in track_data:
                            # event_item bisa jadi tuple atau objek event
                            if isinstance(event_item, tuple) and len(event_item) > 2 and hasattr(event_item[2], 'pitch_wheel_value'):
                                event_obj = event_item[2]
                                if isinstance(event_obj.pitch_wheel_value, float):
                                    logger.critical(f"CRITICAL: INTERNAL MIDI OBJECT STILL HAS FLOAT PITCH WHEEL VALUE! Track {track_idx}, Time {event_item[1]}, Value {event_obj.pitch_wheel_value}")
                            elif hasattr(event_item, 'pitch_wheel_value'): # Jika event_item langsung objek event
                                if isinstance(event_item.pitch_wheel_value, float):
                                     logger.critical(f"CRITICAL: INTERNAL MIDI OBJECT STILL HAS FLOAT PITCH WHEEL VALUE! Track {track_idx}, Event {event_item}, Value {event_item.pitch_wheel_value}")
            else:
                logger.warning("Could not access midi._tracks for deep inspection. MidiUtil version too obscure?")
                
        except Exception as debug_error:
            logger.error(f"Debug error during post-failure internal inspection: {debug_error}")
...
def main_app_runner():
    ...
    logger.info("  FIXED: MIDI pitch bend float-to-int conversion + quantized timing")
