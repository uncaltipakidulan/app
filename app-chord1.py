...
import subprocess
import tempfile
from textblob import TextBlob # Pastikan TextBlob masih digunakan atau hapus jika tidak
...
def create_midi_file(params, output_path):
    ...
    bass_track.append(Message('control_change', channel=2, control=10, value=64, time=0))  # Pan LEFT (30)
...
<body class="bg-gray-100 min-h-screen flex flex-col items-center justify-center p-4 font-sans antialiased">
    <div class="bg-white p-8 rounded-lg shadow-xl w-full max-w-2xl border border-gray-200">
        ...
        <form id="musicForm">
            ...
            <div class="flex items-center justify-center mb-6">
                <button type="submit" id="generateBtn" class="bg-blue-600 hover:bg-blue-700 px-6 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition duration-150 ease-in-out flex items-center justify-center">
        ...
        <div id="audioSection" class="border-t border-gray-200 pt-6 mt-6">
            ...
            <div class="metadata-grid">
                <h3 class="text-sm font-semibold text-gray-700 mb-2">Informasi Instrumental :</h3>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            const musicForm = document.getElementById('musicForm'); // Ambil form
            ...
            const genreSelect = document.getElementById('genreSelect'); // Diperlukan di sini juga
            const tempoInput = document.getElementById('tempoInput');   // Diperlukan di sini juga
            const waveformContainer = document.getElementById('waveform'); // Diperlukan di sini juga
            ...
            // Debug displays (for your internal use, can be removed later)
            // Pastikan elemen debugAudioPlayerId dan debugGenreDisplayId benar-benar ada di HTML
            const debugAudioPlayerId = document.getElementById('debugAudioPlayerId');
            if (debugAudioPlayerId) debugAudioPlayerId.textContent = audioPlayer ? audioPlayer.id : 'N/A (null)';
            const debugGenreDisplayId = document.getElementById('debugGenreDisplayId');
            if (debugGenreDisplayId) debugGenreDisplayId.textContent = genreDisplay ? genreDisplay.id : 'N/A (null)';
            ...
            musicForm.addEventListener('submit', async function(e) {
                ...
                console.log('Generate button clicked (form submitted).'); // Debug log

                const formData = new FormData();
                formData.append('lyrics', lyricsInput.value.trim());
                formData.append('genre', genreSelect.value); // Gunakan genreSelect
                formData.append('tempo', tempoInput.value === '' ? 'auto' : tempoInput.value); // Gunakan tempoInput
                ...
                try {
                    ...
                    const timeoutId = setTimeout(() => {
                        ...
                        console.log('Request timed out after 5 minutes.'); // Debug log timeout
                    }, 5 * 60 * 1000); 

                    console.log('Sending fetch request...'); // Debug log sebelum fetch
                    const response = await fetch('/generate-instrumental', {
                        method: 'POST',
                        body: formData,
                        signal: controller.signal
                    });
                    console.log('Fetch request completed.'); // Debug log setelah fetch
                    ...
                    if (response.ok) {
                        ...
                        console.log('Server response (success):', data); // Debug log data sukses
                        ...
                        if (data.success) {
                            ...
                            wavesurfer = Wavesurfer.create({
                                container: '#waveform',
                                waveColor: '#9ca3af', // Warna gelombang abu-abu
                                progressColor: '#4f46e5', // Warna progress biru/indigo
                                height: 80,
                                barWidth: 2,
                                barRadius: 2,
                                cursorWidth: 1,
                                backend: 'MediaElement',
                                mediaControls: true, // Gunakan kontrol media HTML5 native
                            });
                            ...
                            // Hubungkan Wavesurfer dengan audioPlayer
                        ...
                        console.error('Server response (error):', errorData); // Debug log error
                    ...
                    if (error.name === 'AbortError') {
                        ...
                        console.warn('Request aborted due to timeout.'); // Debug log timeout AbortError
                    } else {
                        statusMsg.innerHTML = `
                        ...
                        `;
                        console.error('Generate error:', error); // Debug log error
                    }
                }
            });
...
def main_app_runner():
    ...
    logger.info("DEBUG: Inside main_app_runner() function.")
    ...
    try:
        ...
        logger.info("DEBUG: main_app_runner() completed successfully.")
        return True
...
if __name__ == '__main__':
    logger.info("DEBUG: Script started, checking main_app_runner.")
    if not main_app_runner():
        logger.error("DEBUG: main_app_runner returned False, exiting.")
        sys.exit(1) # Exit if startup failed
    logger.info("DEBUG: main_app_runner returned True, running app.")
    app.run(host='0.0.0.0', port=5000, debug=False, threaded=True)
    logger.info("DEBUG: Flask app finished running.")
